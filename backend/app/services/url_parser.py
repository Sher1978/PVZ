...
async def resolve_search_query(query: str) -> str:
    ...
    Otherwise, translate Russian search queries into optimal Vietnamese/English marketplace search terms.
    """
    q_clean = query.strip()
    if not q_clean:
        return q_clean
    ...
    if is_url:
        ...
        try:
            path_slug = urlparse(target_url).path.split("/")[-1]
    ...
    # For text queries in Russian / English / Vietnamese, run through multilingual translator
    return translate_query_for_marketplaces(q_clean)
